...
class API:
    exclude = ['generateURLPatterns', 'callAPI', 'getRequestParser']
    views: Dict = dict()
        ...
        def viewFunc(viewSelf, request, *args, **kwargs):
            ...
            if len(params) > 0:
                ...
                if requestParser is not None and issubclass(requestParser, (Serializer, ModelSerializer)):
                    parser = requestParser(data=request.GET if method == Method.GET else request.data)
                    if parser.is_valid():
                        for key, value in parser.validated_data.items():
                            setattr(parser, key, value)
                        if 'request' in params:
                            return func(parser, *args, **kwargs, request=request)
                        else:
