...
class View:
        ...
        def viewFunc(viewSelf, request, *args, **kwargs):
            ...
            if len(params) > 0:
                if 'request' in params:
                    return func(*args, **kwargs, request=request)
                else:
    ...
    def getPermissions(self, className: str, request) -> list:
        if className in self.permissions.keys() and request.method.lower() in self.permissions[className].keys():
    ...
    def generateURLPatterns(self, app_name: str = NotImplemented):
        ...
        for fn in funcs:
            ...
            if hasattr(func, 'method') and hasattr(func, 'path'):
                ...
                if path not in self.views.keys():
                    self.views[path] = type(
                        Utils.urlToClassName(path), (views.View,), {
                            ...
                            'get_permissions':
                                lambda viewSelf: self.getPermissions(Utils.urlToClassName(path), viewSelf.request)
                        }
                    )
                ...
                setattr(self.views[path], method.name.lower(), self.callView(func))
    ...
    def render(self, request, context: dict = NotImplemented, template_name: str = NotImplemented):
        ...
        context['LANGUAGE_NAME'] = get_language_info(getattr(settings, 'LANGUAGE_CODE',
                                                             {'name_local': 'English'})).get('name_local', 'English')
